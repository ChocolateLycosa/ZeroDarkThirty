...
void DX::DeviceResources::CreateDeviceResources() 
{
	...
	HRESULT hr = D3D11CreateDevice(
		...
		D3D11_SDK_VERSION,			// Toujours définir ce paramètre sur la valeur D3D11_SDK_VERSION pour les applications Windows Store.
...
// Cette méthode est appelée lorsque CoreWindow est créé (ou recréé).
...
void DX::DeviceResources::SetDpi(float dpi)
{
	if (dpi != m_dpi)
	{
		...
		// Lorsque l’affichage DPI est modifié, la taille logique de la fenêtre (exprimée en pixels indépendants du périphérique (DIP)) est également modifiée et doit être mise à jour.
...
void DX::DeviceResources::Present() 
{
	...
	// remplacée. Si dirty ou scroll est utilisé, cet appel doit être supprimé.
